...
5분 뒤 진동. 폰을 들었더니 vault에 챕터별 요약 노트 세 장이 떠 있었어요. **내 손은 링크 하나만 던졌는데, 맥미니는 그 5분 동안 80장을 읽고 챕터를 나누고 요약을 짜고 인덱스까지 갱신했더라고요.** 손가락 끝이 살짝 떨렸습니다.
...
- **`--output-format stream-json`** — Claude의 사고·도구호출·결과를 한 줄짜리 JSON으로 흘려보내고, 봇이 그걸 파싱해서 "🔧 Edit: bot.py" 같은 사람 말로 폰에 중계합니다. **봇의 정체가 여기서 드러나요 — 봇은 두뇌가 아니라 번역기예요.**
...
이 글 보고 *"나도 폰에서 맥미니 부려보고 싶은데..."* 싶으시면 정확히 이 다섯 단계입니다. 합쳐 16분 정도, 막힐 거 감안해도 30분 안에 첫 응답 받으실 수 있어요.
...
4. **`launchctl load ~/Library/LaunchAgents/<라벨>.plist`** — 부팅 시 자동 시작 + 죽으면 부활 (2분)
5. **폰에서 "안녕"** — 응답 오면 성공. 안 오면 위 체크리스트 ⛵ (1분)
...
두뇌(맥미니의 Claude)는 한 자리에 두고, 그 두뇌에 닿는 손만 폰·노트북·브라우저로 세 개로 늘렸어요. 그러니까 어디서 키보드를 두드리든 결국 같은 두뇌가 일을 합니다. **좋은 인터페이스는 본체를 잊게 한다 — 맥미니는 보이지 않을 때 가장 잘 일했어요.**
...
뒤에서 깨달은 건 **도구를 늘리는 게 아니라, 내가 있어야 할 자리를 줄이는 일이었다**는 점이에요. 일·도구·내 시간에 대한 작은 태도 하나가 같이 바뀐 한 주였습니다.

**강의에 살짝 아쉬웠던 건** — 봇 만들기까지는 잘 따라갔는데, 봇 운영 후 안정성·디버깅(launchd 재시작 타이밍, 토큰 만료 같은 운영 함정) 얘기가 없어서 일주일 동안 혼자 부딪히며 알아갔어요. 3주차 Tailscale + API 들어가기 전에 한 줄이라도 짚어주시면 좋겠다는 생각이.

다음 주는 **시스템 UI(3주차) — Tailscale + API**. 이번 주 SSH/ttyd가 이미 Tailscale 위에서 굴렀으니 도입은 부드러울 것 같고, API까지 더해지면 다음 단계 그림이 나옵니다. 4주차 MCP까지 가면 헤르메스·코덱스까지 한 호흡으로 묶이는 그림이 진짜 완성될 것 같아요.

다음 주에 뵙겠습니다.

> **맥미니는 더 이상 책상 위에 있지 않아요. 제 주머니 안에 있습니다.**

여러분의 본체는 어디 있나요? 다리는 몇 개인가요? 🙌
